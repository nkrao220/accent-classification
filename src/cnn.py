...
X_train = np.load('X_train_moz.npy').reshape(-1, 16, 64, 1)
X_test = np.load('X_test_moz.npy').reshape(-1, 16, 64, 1)
X_val = np.load('X_val_moz.npy').reshape(-1, 16, 64, 1)
...
model.compile(loss=keras.losses.binary_crossentropy,
              ...
              metrics=['accuracy'])
model.fit(X_train_moz, y_train_moz, batch_size=512, epochs=1500, verbose=1,
            validation_data=(X_test_moz, y_val_moz))
